...
def verify_first_message_correctness(
    ...
    """Can be used to enforce that the first message always uses send_message_remote"""
    ...
    if (hasattr(response_message, "function_call") and response_message.function_call is not None) and (
        ...
        print(f"First message includes both function call AND tool call: {response_message}")
        return False
        ...
        print(f"First message didn't include function call: {response_message}")
        return False

    function_name = function_call.name if function_call is not None else ""
    if require_send_message and function_name != "send_message_remote" and function_name != "archival_memory_search":
        print(f"First message function call wasn't send_message or archival_memory_search: {response_message}")
        return False

    if require_monologue and (not response_message.content or response_message.content is None or response_message.content == ""):
        print(f"First message missing internal monologue: {response_message}")
        return False
    ...
    if response_message.content:
        ...
        if contains_special_characters(monologue):
            print(f"First message internal monologue contained special characters: {response_message}")
            return False
        # if 'functions' in monologue or 'send_message' in monologue or 'inner thought' in monologue.lower():
        if "functions" in monologue or "send_message_remote" in monologue:
            # Sometimes the syntax won't be correct and internal syntax will leak into message.context
            print(f"First message internal monologue contained reserved words: {response_message}")
            return False
