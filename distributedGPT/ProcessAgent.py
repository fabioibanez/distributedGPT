...
class ProcessAgent(Agent):
    ...
    def process_agent_step(agent: ProcessAgent, user_message, no_verify = False):
        ...
        if token_warning:
            user_message = MemGPT.memgpt.system.system.get_token_limit_warning()
            skip_next_user_input = True
        elif function_failed:
            user_message = MemGPT.memgpt.system.system.get_heartbeat(FUNC_FAILED_HEARTBEAT_MESSAGE)
            skip_next_user_input = True
        elif heartbeat_request:
            user_message = MemGPT.memgpt.system.system.get_heartbeat(REQ_HEARTBEAT_MESSAGE)
            skip_next_user_input = True
